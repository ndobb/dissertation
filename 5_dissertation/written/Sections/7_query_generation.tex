...
\noindent With the difference of only the inequality operator ($\geq$ versus $<$) the evaluation query has a relatively high BLEU score of 70.1\%, despite returning zero correct patients! Evaluating instead using query results would return zero patients matched and thus 0\% recall. For these reasons, we found comparison of query results rather than syntax to be a more meaningful and useful metric.
